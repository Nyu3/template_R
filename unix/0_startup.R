...
## Reading data == (2020-10-19) ================================================
getData. <- function(filePath = NULL, file = NULL, timeSort = F, timeFactor = NULL, type = NULL, sheet_bind = T, ...) {
    oldDir <- getwd()
...
## Optimum position of y-axis label == (2020-10-12) ================================================
yPos. <- function(Ylim2, ...) {
    axisFun.(Ylim2, n = 6)[[1]] %>% {.[between(., Ylim2[1], Ylim2[2])]} %>% {strwidth(.) /strwidth('|||')} %>% #{max(ceiling(.))}
    max(.) %>% whichSize.(vec = c(1, 2, 2.5, 3, 3.5, 4), ref = ., c(2.2, 1.8, 1.6, 1.4, 1.1, 0.9))  # same adjust; -0.1, 0.1, 100
}
...
## Write list data to csv/xlsx file == (2020-10-12) ================================================
...
write2. <- function(dL, name = NULL, sheet = NULL, ...) {
...
## Boxplot oriented for quantile limit and full/half box == (2020-10-13) ================================================
